...
class OrderPdfService {
  ...
  static Future<void> generateAndDownloadOrderPdf(OrderModel order) async {
    ...
    final invoiceNo  = 'INV-${order.id.substring(0, 8).toUpperCase()}';
    ...
    final gstAmount  = order.gstAmount;
    final delivery   = order.deliveryCharge;
    final grandTotal = order.totalAmount;
  ...
  // ─── Bill-To block ────────────────────────────────────────────────────────
  static pw.Widget _buildBillTo(OrderModel order) {
    return pw.Container(
  ...
  static pw.Widget _buildOrderMeta(OrderModel order, bool isPaid, DateFormat dateFmt) {
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(height: 6),
          _metaRow('Order ID',   '#${order.id.substring(0, 8)}'),
  ...
  static PdfColor _statusColor(String status) {
    switch (status.toLowerCase()) {
      ...
      case 'cancelled':
      case 'returned':  return const PdfColor.fromInt(0xFFEF4444);
      case 'confirmed': return _accent;
      case 'processing':return const PdfColor.fromInt(0xFF0EA5E9);
      case 'in_transport': return const PdfColor.fromInt(0xFF8B5CF6);
      default:          return _textMuted;
    }
  }

  // ─── Items table ──────────────────────────────────────────────────────────
  static pw.Widget _buildItemsTable(List<OrderItemModel> items, NumberFormat fmt) {
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    if (item.qualityOptionName.isNotEmpty)
                      pw.Text(item.qualityOptionName, style: const pw.TextStyle(fontSize: 7, color: _textMuted)),
                  ],
                ),
              ),
              cell('$qty$unit',
